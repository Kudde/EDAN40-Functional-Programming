...
-- = expr if all the variables occur in dictionary and there is no division by zero. Otherwise an error should be reported using error
value :: Expr -> Dictionary.T String Integer -> Integer
value (Num n) d   = n
value (Var v) d   = fromMaybe (error "Please define all variables..") (Dictionary.lookup v d)
